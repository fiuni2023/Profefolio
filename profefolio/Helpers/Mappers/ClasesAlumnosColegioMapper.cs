...
namespace profefolio.Helpers.Mappers
{
    public class ClasesAlumnosColegioMapper : Profile
    {
        public ClasesAlumnosColegioMapper()
        {
            ...
            .ForMember(dest => dest.Nombre,
                opt => opt.MapFrom(
                    src => src.Persona == null ? "" : src.Persona.Nombre))
            .ForMember(dest => dest.Apellido,
                opt => opt.MapFrom(
                    src => src.Persona == null ? "" : src.Persona.Apellido))
            .ForMember(dest => dest.Documento,
                opt => opt.MapFrom(
                    src => src.Persona == null ? "" : src.Persona.Documento))
            .ForMember(dest => dest.DocumentoTipo,
                opt => opt.MapFrom(
                    src => src.Persona == null ? "" : src.Persona.DocumentoTipo))
            .ForMember(dest => dest.Direccion,
                opt => opt.MapFrom(
                    src => src.Persona == null ? "" : src.Persona.Direccion))
            .ForMember(dest => dest.Id,
                opt => opt.MapFrom(
                    src => src.Persona == null ? "" : src.Persona.Id))
            .ForMember(dest => dest.Nacimiento,
                opt => opt.MapFrom(
                    src => src.Persona == null ? new DateTime() : src.Persona.Nacimiento))
            .ForMember(dest => dest.Email,
                opt => opt.MapFrom(
                    src => src.Persona == null ? "" : src.Persona.Email))
